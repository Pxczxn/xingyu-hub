import { request, apiRecord } from '@/utils/api-request'
...
export const authApi = {
  ...
  getCaptcha(): Promise<CaptchaResult> {
    return apiRecord<CaptchaResult>({
      url: '/auth/captcha',
      method: 'get'
    })
  },
  ...
  async login(data: LoginParams): Promise<LoginResult> {
    ...
    return request({
      ...
      method: 'post',
      data: encryptedData
    }).then((payload) => asApiRecord<LoginResult>(payload))
  },
  ...
  getInfo(): Promise<UserInfoResult> {
    return apiRecord<UserInfoResult>({
      url: '/auth/info',
      method: 'get'
    })
  },
