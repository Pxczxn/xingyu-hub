...
export const noticeApi = {
  ...
  page(params: { page: number; pageSize: number; title?: string; noticeType?: number; status?: number }): Promise<PageResult<SysNotice>> {
    return apiPage<SysNotice>({ url: '/sys/notice/page', method: 'get', params })
  },
  
  // 获取当前用户通知
  myNotices(params: { page: number; pageSize: number; isRead?: number }): Promise<PageResult<SysNotice>> {
    return apiPage<SysNotice>({ url: '/sys/notice/my', method: 'get', params })
  },
  
  // 获取详情（含 channels、targetIds 数组）
  detail(id: number): Promise<SysNotice & { channels?: string[]; targetIds?: number[] }> {
    return apiRecord<SysNotice & { channels?: string[]; targetIds?: number[] }>({ url: `/sys/notice/${id}`, method: 'get' })
  },
  ...
  getUnreadCount(): Promise<number> {
    return apiValue<number>({ url: '/sys/notice/unread-count', method: 'get' })
  },
  // 获取可用推送渠道（从 sys_config_group 读取）
  getChannels(): Promise<NoticeChannelOption[]> {
    return apiList<NoticeChannelOption>({ url: '/sys/notice/channels', method: 'get' })
  },

  // 获取通知推送记录（触达情况）
  getSendLogs(id: number): Promise<NoticeSendLog[]> {
    return apiList<NoticeSendLog>({ url: `/sys/notice/${id}/send-logs`, method: 'get' })
  },
...
export const chatApi = {
  ...
  send(data: { receiverId: number; content: string; msgType?: number }): Promise<ChatMessage> {
    return request({ url: '/sys/chat/send', method: 'post', data }).then((payload) => apiRecord<ChatMessage>(payload))
  },
  
  // 获取聊天记录
  getHistory(targetId: number, params: { page: number; pageSize: number }): Promise<PageResult<ChatMessage>> {
    return apiPage<ChatMessage>({ url: `/sys/chat/history/${targetId}`, method: 'get', params })
  },
  
  // 获取最近联系人
  getContacts(): Promise<ChatMessage[]> {
    return apiList<ChatMessage>({ url: '/sys/chat/contacts', method: 'get' })
  },
  
  // 获取用户列表
  getUsers(): Promise<ChatUser[]> {
    return apiList<ChatUser>({ url: '/sys/chat/users', method: 'get' })
  },
  ...
  // 获取未读数量
  getUnreadCount(): Promise<number> {
    return apiValue<number>({ url: '/sys/chat/unread-count', method: 'get' })
  },
  
  // 检查用户是否在线
  isOnline(userId: number): Promise<boolean> {
    return apiValue<boolean>({ url: `/sys/chat/online/${userId}`, method: 'get' })
  },
  ...
  getBlacklist(): Promise<any[]> {
    return apiList<any>({ url: '/sys/chat/blacklist', method: 'get' })
  },
  
  // 检查是否拉黑
  isBlocked(targetId: number): Promise<boolean> {
    return apiValue<boolean>({ url: `/sys/chat/blocked/${targetId}`, method: 'get' })
  }
}
...
export const groupChatApi = {
  ...
  list(): Promise<ChatGroup[]> {
    return apiList<ChatGroup>({ url: '/chat/group/list', method: 'get' })
  },
  
  // 获取群详情
  detail(groupId: number): Promise<ChatGroup> {
    return apiRecord<ChatGroup>({ url: `/chat/group/${groupId}`, method: 'get' })
  },
  ...
  members(groupId: number): Promise<ChatGroupMember[]> {
    return apiList<ChatGroupMember>({ url: `/chat/group/${groupId}/members`, method: 'get' })
  },
  ...
  getMessages(groupId: number, page: number = 1, pageSize: number = 50): Promise<PageResult<ChatGroupMessage>> {
    return apiPage<ChatGroupMessage>({ url: `/chat/group/${groupId}/messages`, method: 'get', params: { page, pageSize } })
  }
}
