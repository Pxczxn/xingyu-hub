...
import { adminNavigation, buildNavigationBreadcrumbs, flattenNavigationIcons } from '@/config/navigation'
...
function getIconName(key: string): string {
  const navigationIcons = flattenNavigationIcons()
  if (navigationIcons[key]) return navigationIcons[key]

  const iconMapping: Record<string, string> = {
    '/message/chat': 'ChatbubbleOutline'
  }
  return iconMapping[key] || navigationIcons[key] || 'MenuOutline'
}
...
function navigationItem(label: string, key: string, icon: string): MenuOption {
...
const menuOptions = computed<MenuOption[]>(() =>
  adminNavigation.map((node) => {
    if (node.type === 'item') {
      return navigationItem(node.label, node.key, node.icon)
    }
    ...
    return {
      ...
      children: node.children.map((child) => navigationItem(child.label, child.key, child.icon))
    }
  })
)
...
// 面包屑
const breadcrumbs = computed(() => buildNavigationBreadcrumbs(route.path))
...
.layout {
  ...
  --admin-header-h: 60px;
  --admin-tabbar-h: 0px;
  --admin-chrome-h: var(--admin-header-h);

  &.has-tabs {
    --admin-tabbar-h: 38px;
    --admin-chrome-h: calc(var(--admin-header-h) + var(--admin-tabbar-h));
  }
