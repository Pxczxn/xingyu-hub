<template>
  <div class="system-workspace page-container">
    <header class="admin-page-heading">
      <div><p class="admin-page-heading__kicker">MESSAGE DELIVERY</p><h1>系统通知</h1><span class="admin-page-heading__description">创建并跟踪站内、邮件、短信和 Webhook 通知。</span></div>
    </header>
    <n-card :bordered="false" class="page-layout page-list-card">
    ...
    >
      <n-data-table
        v-bind="tableListProps(sendLogScroll)"
        :columns="sendLogColumns"
        :data="sendLogsData"
        :loading="sendLogsLoading"
        :row-key="(row: NoticeSendLog) => row.id || row.channel + row.sendTime"
      />
      <EmptyState v-if="!sendLogsLoading && sendLogsData.length === 0" description="该通知暂无推送记录" />
    </n-modal>
  </div>
</template>
...
const columns: DataTableColumns<SysNotice> = [
  ...
  { title: '标题', key: 'title', ...colLayout('title'), render: (row) => renderTableLink(row.title || '(无标题)', () => handleView(row)) },
  {
    title: '类型',
    key: 'noticeType',
    ...colLayout('type'),
