<template>
  <div class="system-workspace page-container page-container--scroll">
    <header class="admin-page-heading">
      <div><p class="admin-page-heading__kicker">TRAFFIC INSIGHT</p><h1>API 访问</h1><span class="admin-page-heading__description">分析接口流量、成功率、请求方法和访问趋势。</span></div>
    </header>
    <n-card class="page-layout">
      ...
      <n-data-table v-bind="tableListProps(tableScroll)" :columns="columns" :data="tableData" :loading="loading" :row-key="(row: ApiAccessLog) => row.id" />
...
const columns: DataTableColumns<ApiAccessLog> = [
  ...
  { title: 'API 路径', key: 'apiPath', ...colLayout('title') },
  { title: '方法', key: 'method', ...colLayout('type') },
  { title: '状态码', key: 'statusCode', ...colLayout('number') },
  { title: '成功', key: 'success', ...colLayout('boolean'), render: (row) => row.success === 1 ? '是' : '否' },
