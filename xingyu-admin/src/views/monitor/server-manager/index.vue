<template>
  <div class="system-workspace page-container page-container--scroll">
    <header class="admin-page-heading">
      <div><p class="admin-page-heading__kicker">SERVER ACCESS</p><h1>服务器管理</h1><span class="admin-page-heading__description">维护受控服务器连接并执行连通性检查。</span></div>
      <div class="admin-page-heading__actions"><span class="monitor-count">{{ servers.length }} 台服务器</span><n-button type="primary" @click="handleAdd"><template #icon><n-icon><AddOutline /></n-icon></template>新增服务器</n-button></div>
    </header>
    <!-- 操作栏 -->
...
function initTerminal(server: Server) {
  ...
  const wsHost = isDev ? 'localhost:7779' : location.host
