<template>
  <div class="system-workspace page-container page-container--scroll">
    <header class="admin-page-heading">
      <div><p class="admin-page-heading__kicker">PLATFORM SETTINGS</p><h1>系统配置</h1><span class="admin-page-heading__description">集中管理站点、注册、登录、存储和消息服务配置。</span></div>
      <div class="admin-page-heading__actions"><n-button secondary @click="handleRefresh">刷新缓存</n-button><n-button type="primary" :loading="saving" @click="handleSave">保存配置</n-button></div>
    </header>
          ...
          <div class="config-content">
            ...
            <template v-if="group.groupCode === 'system'">
              <n-form :model="configs.system" label-placement="left" label-width="120">
                ...
                <n-form-item label="社区前台地址">
                  <n-input v-model:value="configs.system.communityWebUrl" placeholder="http://127.0.0.1:7777" style="width: 400px" />
                  <span class="form-hint">用于邮件验证、密码重置链接</span>
                </n-form-item>
                <n-divider />
            ...
            <template v-else-if="group.groupCode === 'register'">
              <n-alert type="info" style="margin-bottom: 16px" :bordered="false">
                以下配置控制社区前台（xingyu-web）的用户注册，管理端不提供自助注册。
              </n-alert>
              <n-form :model="configs.register" label-placement="left" label-width="140">
                <n-form-item label="开放社区注册">
                  <n-switch v-model:value="configs.register.enabled" />
                </n-form-item>
                <n-form-item label="登录前验证邮箱">
                  <n-switch v-model:value="configs.register.verifyEmail" />
                  <span class="form-hint">开启后须完成邮箱验证才能登录</span>
                </n-form-item>
                <n-form-item label="手机验证">
                  <n-switch v-model:value="configs.register.verifyPhone" />
                  <span class="form-hint">须先在「短信配置」启用短信并填写服务商密钥；未配置时社区注册不会要求手机号</span>
                </n-form-item>
                <n-form-item label="需要审核">
                  <n-switch v-model:value="configs.register.needAudit" />
                  <span class="form-hint">注册后须管理员在「社区用户」中审核通过才能登录</span>
                </n-form-item>
                <n-form-item label="默认角色">
                  <n-input v-model:value="configs.register.defaultRole" placeholder="user" style="width: 200px" />
                  <span class="form-hint">新用户注册后分配的角色标识（小写字母开头）</span>
                </n-form-item>
              </n-form>
            </template>
            ...
            <template v-else-if="group.groupCode === 'login'">
              <n-alert type="info" style="margin-bottom: 16px" :bordered="false">
                验证码与重试锁定同时作用于管理端与社区前台。社区端统一使用图形/算术验证码；管理端可选滑块或短信验证码（短信须先配置「短信配置」）。
              </n-alert>
              <n-form :model="configs.login" label-placement="left" label-width="120">
                ...
                <n-form-item label="单点登录">
                  <n-switch v-model:value="configs.login.singleLogin" />
                  <span class="form-hint">开启后新登录会踢掉同账号的其他会话（管理端与社区均生效）</span>
                </n-form-item>
            ...
            <template v-else-if="group.groupCode === 'password'">
              ...
              <n-form :model="configs.password" label-placement="left" label-width="150">
                ...
                <n-form-item label="密码过期天数">
                  ...
                  <span class="form-hint">0 表示永不过期；仅管理端登录时校验</span>
                </n-form-item>
              </n-form>
            </template>
            ...
            <template v-else-if="group.groupCode === 'email'">
              <n-alert type="info" style="margin-bottom: 16px" :bordered="false">
                SMTP 配置同时用于管理端与社区（邮箱验证、密码重置）。下方「邮件模板」中验证码类用于管理端，带链接的模板用于社区。
              </n-alert>
            ...
            <template v-else-if="group.groupCode === 'emailTemplate'">
              <n-alert type="info" style="margin-bottom: 16px" :bordered="false">
                验证码类模板用于管理端；带链接的模板用于社区邮箱验证与密码重置。
              </n-alert>
              <n-form :model="configs.emailTemplate" label-placement="top">
                ...
                <n-form-item label="重置密码邮件模板（验证码）">
                  <n-input v-model:value="configs.emailTemplate.resetPassword" type="textarea" :rows="3" placeholder="支持变量: {code}, {expire}" />
                </n-form-item>
                <n-form-item label="社区邮箱验证模板">
                  <n-input v-model:value="configs.emailTemplate.verifyEmail" type="textarea" :rows="3" placeholder="支持变量: {link}, {siteName}" />
                </n-form-item>
                <n-form-item label="社区密码重置模板">
                  <n-input v-model:value="configs.emailTemplate.passwordResetLink" type="textarea" :rows="3" placeholder="支持变量: {link}, {siteName}" />
                </n-form-item>
            ...
            <template v-else-if="group.groupCode === 'sms'">
              <n-alert type="warning" style="margin-bottom: 16px" :bordered="false">
                未接入阿里云/腾讯云等通道前请保持「启用短信」关闭。本地调试可将服务商选为「控制台模式」，验证码会输出在后端日志。
              </n-alert>
              <n-alert type="info" style="margin-bottom: 16px" :bordered="false">
                短信配置在此预留（阿里云/腾讯云/七牛等）。当前仅管理端在「登录配置 → 验证码类型 = 短信」且启用短信时可发登录验证码；社区注册手机验证需同时开启本页「启用短信」与注册配置中的「手机验证」。
              </n-alert>
            ...
            <template v-else-if="group.groupCode === 'push'">
              <n-alert type="info" style="margin-bottom: 16px" :bordered="false">
                推送仅用于管理端系统通知（钉钉/飞书/企微 Webhook），与社区无关。
              </n-alert>
            ...
            <template v-else-if="group.groupCode === 'thirdParty'">
              <n-alert type="warning" style="margin-bottom: 16px" :bordered="false">
                此页配置未接入实际登录流程。微信请使用「公众号配置」或「小程序配置」；GitHub/支付宝登录尚未实现。
              </n-alert>
              <n-collapse>
            ...
            <template v-else-if="group.groupCode === 'security'">
              <n-alert type="info" style="margin-bottom: 16px" :bordered="false">
                接口加密、防调试等仅作用于管理端 API 与前端；社区 API 不加密。Token 配置作用于管理端/App 的 Sa-Token，不影响社区会话。
              </n-alert>
    ...
    <n-modal v-model:show="showSmsLogsModal" preset="card" title="短信发送记录" style="width: 800px">
      <n-space vertical>
        ...
        <n-data-table
          v-bind="tableListProps(smsLogsScroll)"
...
const configs = reactive<Record<string, any>>({
  system: { siteName: '', siteDescription: '', siteLogo: '', copyright: '', icp: '', communityWebUrl: 'http://127.0.0.1:7777', watermarkEnabled: true, watermarkType: 'username', watermarkCustomText: '', watermarkOpacity: 0.1 },
  register: { enabled: true, verifyEmail: false, verifyPhone: false, needAudit: false, defaultRole: 'user' },
  ...
  emailTemplate: { verifyCode: '', resetPassword: '', verifyEmail: '', passwordResetLink: '', welcome: '' },
  sms: { provider: 'console', accessKeyId: '', accessKeySecret: '', signName: '', tencentAppId: '', templateVerifyCode: '', templateResetPassword: '', templateNotice: '', enabled: false },
  ...
  security: {
    encryptEnabled: false, encryptScope: 'partial', encryptProtocol: 'XYC1', encryptPublicKey: '', xssFilter: true, sqlInject: true,
    disableDevtool: false, // 前端禁止调试
    // Token 配置
    tokenName: 'satoken', tokenTimeout: 86400, tokenActiveTimeout: 86400,
...
const smsProviderOptions = [
  { label: '控制台模式（开发调试，验证码打日志）', value: 'console' },
...
const smsLogsColumns = [
  { title: '手机号', key: 'phone', ...colLayout('phone') },
  { title: '验证码', key: 'content', ...colLayout('slug') },
  { title: '服务商', key: 'provider', ...colLayout('type') },
  {
    title: '状态',
    key: 'status',
    ...colLayout('status'),
...
async function loadGroups() {
  try {
    ...
    if (configGroups.value.length > 0) {
      ...
      activeTab.value = configGroups.value.some(group => group.groupCode === requestedTab)
        ? requestedTab
        : configGroups.value[0].groupCode
...
async function handleSave() {
  ...
  try {
    ...
    if (activeTab.value === 'system') {
      ...
      siteStore.siteName = configs.system.siteName || '星语管理端'
