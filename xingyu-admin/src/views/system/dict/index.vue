<template>
  <div class="system-workspace page-container">
    <header class="admin-page-heading">
      <div><p class="admin-page-heading__kicker">DATA DICTIONARY</p><h1>字典管理</h1><span class="admin-page-heading__description">统一维护业务枚举、显示标签与启用状态。</span></div>
    </header>
    ...
    <n-modal v-model:show="dataModalVisible" title="字典数据" preset="card" style="width: 900px" :mask-closable="false">
      ...
      <n-data-table v-bind="tableListProps(dictDataScroll)" :columns="dataColumns" :data="dictDataList" :loading="dataLoading" />
    </n-modal>
...
const columns: DataTableColumns<SysDictType> = [
  ...
  { title: '字典名称', key: 'dictName', ...colLayout('name') },
...
const dataColumns: DataTableColumns<SysDictData> = [
  { title: '排序', key: 'sort', ...colLayout('sort') },
  { title: '字典标签', key: 'dictLabel', ...colLayout('name') },
  { title: '字典键值', key: 'dictValue', ...colLayout('slug') },
  { title: '回显样式', key: 'listClass', ...colLayout('type'), render(row) {
