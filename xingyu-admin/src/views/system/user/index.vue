<template>
  <div class="system-workspace page-container">
    <header class="admin-page-heading">
      <div><p class="admin-page-heading__kicker">ACCESS MANAGEMENT</p><h1>系统用户</h1><span class="admin-page-heading__description">管理后台账号、组织归属、角色和登录状态。</span></div>
    </header>
    <!-- 用户列表 -->
    <n-card class="user-list-card page-list-card" size="small">
...
const columns: DataTableColumns<SysUser> = [
  ...
  { title: '用户名', key: 'username', ...colLayout('name') },
  { title: '昵称', key: 'nickname', ...colLayout('name') },
  { title: '部门', key: 'deptName', ...colLayout('name'), render(row) {
    return row.deptName || '-'
  }},
  { title: '岗位', key: 'postNames', ...colLayout('description'), render(row) {
  ...
  { title: '手机号', key: 'phone', ...colLayout('phone'), render(row) {
      return row.phone || '-'
    }},
  {
    title: '离职',
