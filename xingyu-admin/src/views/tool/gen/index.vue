<template>
  <div class="system-workspace page-container">
    <header class="admin-page-heading">
      <div><p class="admin-page-heading__kicker">DEVELOPER TOOLING</p><h1>代码生成</h1><span class="admin-page-heading__description">导入数据表并维护代码生成配置。</span></div>
    </header>
    ...
    <n-modal v-model:show="showImportModal" preset="card" title="导入表" style="width: 900px">
      ...
      <n-spin :show="importLoading">
        <n-data-table
          v-bind="tableListProps(importScroll)"
    ...
    <n-modal v-model:show="showEditModal" preset="card" title="编辑生成配置" style="width: 1100px">
      <n-tabs type="line">
        ...
        <n-tab-pane name="columns" tab="字段配置">
          <n-data-table v-bind="tableListProps(columnEditScroll)" :columns="columnEditColumns" :data="editForm.columns" :max-height="400" />
        </n-tab-pane>
...
const columnEditColumns: DataTableColumns<GenTableColumn> = [
  { title: '字段名', key: 'columnName', ...colLayout('slug') },
  { title: '字段描述', key: 'columnComment', ...colLayout('name'),
    render(row) {
      return h(NInput, { value: row.columnComment, size: 'small', onUpdateValue: (v: string) => { row.columnComment = v } })
    }
  },
  { title: 'Java类型', key: 'javaType', ...colLayout('type'),
    render(row) {
      return h(NSelect, {
  ...
  { title: '显示类型', key: 'htmlType', ...colLayout('type'),
    render(row) {
      return h(NSelect, {
  ...
  { title: '字典类型', key: 'dictType', ...colLayout('slug'),
    render(row) {
