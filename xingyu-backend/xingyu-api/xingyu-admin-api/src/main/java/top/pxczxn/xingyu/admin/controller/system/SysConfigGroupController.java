...
import top.pxczxn.xingyu.system.config.SaTokenConfigLoader;
...
import top.pxczxn.xingyu.sms.service.SmsLogService;
...
import top.pxczxn.xingyu.pay.PayServiceFactory;
...
import top.pxczxn.xingyu.system.service.impl.CryptoServiceImpl;
...
public class SysConfigGroupController {
    ...
    public Result<List<SysConfigGroup>> list() {
        List<SysConfigGroup> groups = configGroupService.listAll();
        for (SysConfigGroup group : groups) {
            if (GROUP_SECURITY.equals(group.getGroupCode())) {
                maskSecurityConfig(group);
            }
        }
    ...
    public Result<SysConfigGroup> getByCode(@PathVariable String groupCode) {
        ...
        if (GROUP_SECURITY.equals(groupCode) && group != null) {
            maskSecurityConfig(group);
        }
    ...
    public Result<Void> save(@PathVariable String groupCode, @RequestBody Map<String, Object> config) {
        try {
            if (GROUP_SECURITY.equals(groupCode)) {
                config = mergeSecurityConfigOnSave(config);
            }
            String configValue = objectMapper.writeValueAsString(config);
            configGroupService.saveConfig(groupCode, configValue);

            // 存储参数变更时需废弃旧实例；否则同一存储类型下的密钥、域名、路径仍会沿用旧值。
            if ("storage".equals(groupCode)) {
    ...
    /**
     * 生成 RSA 密钥对（私钥仅存服务端，不返回浏览器）
     */
    @PostMapping("/generate-keys")
    public Result<Map<String, String>> generateKeys() {
        try {
            String publicKey = cryptoService.generateAndPersistRsaKeyPair();
    ...
    /**
     * security 配置读取时移除私钥字段。
     * TODO(P2): RSA 私钥迁移至独立密钥存储，不再放在 sys_config_group JSON。
     */
    private void maskSecurityConfig(SysConfigGroup group) {
        if (group == null || group.getConfigValue() == null || group.getConfigValue().isBlank()) {
            return;
        }
        try {
            ObjectNode node = (ObjectNode) objectMapper.readTree(group.getConfigValue());
            node.remove("encryptPrivateKey");
            group.setConfigValue(objectMapper.writeValueAsString(node));
    ...
    /**
     * 保存 security 时若前端未携带私钥，保留库中已有私钥。
     */
    private Map<String, Object> mergeSecurityConfigOnSave(Map<String, Object> incoming) {
        Object privateKey = incoming.get("encryptPrivateKey");
        if (privateKey != null && !privateKey.toString().isBlank()) {
            return incoming;
        }
        SysConfigGroup existing = configGroupService.getByGroupCode(GROUP_SECURITY);
        if (existing == null || existing.getConfigValue() == null || existing.getConfigValue().isBlank()) {
            incoming.remove("encryptPrivateKey");
            return incoming;
        }
        try {
            JsonNode existingNode = objectMapper.readTree(existing.getConfigValue());
            JsonNode existingPrivateKey = existingNode.get("encryptPrivateKey");
            if (existingPrivateKey != null && !existingPrivateKey.isNull()) {
                incoming.put("encryptPrivateKey", existingPrivateKey.asText());
            } else {
                incoming.remove("encryptPrivateKey");
            }
        } catch (Exception e) {
