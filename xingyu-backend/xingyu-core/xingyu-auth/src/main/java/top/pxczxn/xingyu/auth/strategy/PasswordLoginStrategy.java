package top.pxczxn.xingyu.auth.strategy;

import cn.hutool.crypto.digest.BCrypt;
import top.pxczxn.xingyu.auth.LoginHelper;
import top.pxczxn.xingyu.auth.LoginRequest;
import top.pxczxn.xingyu.auth.LoginResult;
import top.pxczxn.xingyu.auth.LoginStrategy;
...
public class PasswordLoginStrategy implements LoginStrategy {
    ...
    private void checkPasswordExpired(SysUser user) {
        int expireDays = configHelper.getPasswordExpireDays();
        if (expireDays <= 0) {
            return;
        }
        LocalDateTime changedAt = user.getUpdateTime() != null ? user.getUpdateTime() : user.getCreateTime();
        if (changedAt == null) {
            return;
        }
        if (changedAt.plusDays(expireDays).isBefore(LocalDateTime.now())) {
            loginHelper.recordFailLog(user.getUsername(), "密码已过期");
