...
public class SystemConfigHelper implements top.pxczxn.xingyu.crypto.CryptoConfigProvider, top.pxczxn.xingyu.mail.MailConfigProvider {
    ...
    /**
     * 社区注册是否实际要求手机验证（须同时开启短信服务）
     */
    public boolean isCommunityPhoneVerificationEnabled() {
        return isRegisterVerifyPhone() && isSmsEnabled();
    }
    ...
    public int getPasswordExpireDays() {
        return getInt(GROUP_PASSWORD, "expireDays", 0);
    }
    ...
    public String getStorageLocalPath() {
        return getString(GROUP_STORAGE, "localPath", "./runtime/uploads");
    }
    ...
    public String getTokenName() {
        return getString(GROUP_SECURITY, "tokenName", "satoken");
    }
    ...
    /**
     * 加密协议：XYC1（默认）或 legacy
     */
    public String getEncryptProtocol() {
        return getString(GROUP_SECURITY, "encryptProtocol", "XYC1");
    }
     ...
     * 社区邮箱验证链接模板
     */
    public String getEmailTemplateVerifyEmailLink() {
        return getNonBlankString(
     ...
     * 社区密码重置链接模板
     */
    public String getEmailTemplatePasswordResetLink() {
        return getNonBlankString(
                GROUP_EMAIL_TEMPLATE,
                "passwordResetLink",
                "请点击链接设置新密码：{link}");
    ...
    private String getNonBlankString(String groupCode, String key, String defaultValue) {
        String value = getString(groupCode, key, defaultValue);
    ...
    public String getSiteName() {
        return getString(GROUP_SYSTEM, "siteName", "星语管理端");
    }
    ...
    /**
     * 社区前台地址（用于邮件验证、密码重置链接）
     */
    public String getCommunityWebUrl() {
        String url = getString(GROUP_SYSTEM, "communityWebUrl", "http://127.0.0.1:7777");
        if (url.endsWith("/")) {
    ...
    /**
     * 获取推送服务商：优先使用已配置 Webhook 的平台
     */
    public String getPushProvider() {
        JsonNode config = getConfig(GROUP_PUSH);
        if (config != null) {
            for (String provider : new String[]{"dingtalk", "feishu", "wechat_work"}) {
                JsonNode node = config.get(provider);
                if (node != null && node.has("tokenId") && !node.get("tokenId").asText("").isBlank()) {
                    return provider;
                }
            }
        }
