"use client";
import styles from "./public-collections.module.css";
...
function ProfileAvatar({ name, avatar }: { name: string; avatar?: string | null }) {
  ...
  return <span className={cn(styles.avatarFallback)}>{name.slice(0, 1).toUpperCase()}</span>;
}
...
export default function PublicCollectionsPage() {
  ...
  return (
    <AppShell>
      <main className={cn(styles.collectionsPage)} data-layout="public-collections">
        <aside className={cn(styles.rail)} aria-label="收藏导航">
          <b><Sparkles /> 星语社区</b>
          <nav>
            <Link href="/discover"><Compass />发现</Link>
            <Link href="/me/following"><UserRound />关注</Link>
            <Link href="/me/collections"><Bookmark />收藏</Link>
            <Link className={cn(styles.active)} href="/collections/public"><FolderHeart />公开收藏</Link>
          </nav>
          <Link href="/studio" className={cn(styles.create)}><Sparkles /> 创作中心</Link>
        </aside>
        ...
        <section className={cn(styles.content)}>
          ...
          {collectionsState.loading ? <p className={cn(styles.status)}>正在加载公开收藏夹…</p> : null}
          {collectionsState.error ? <p className={cn(styles.status, styles.statusError)}>{collectionsState.error}</p> : null}
          {!collectionsState.loading && !collectionsState.error && publicCollections.length ? (
            <section className={cn(styles.grid, styles.gridData)}>
              {publicCollections.map((collection, index) => <Link href={`/collections/${encodeURIComponent(collection.id)}`} key={collection.id}>
                <div className={cn(styles.collectionArt, styles[`collectionArt${index % 4}`])}><FolderHeart /></div>
                <div><span>公开收藏夹</span><h3>{collection.title}</h3><p>{collection.itemCount ?? 0} 条内容已收录，持续由创作者整理。</p><small>公开可见</small><i>→</i></div>
              </Link>)}
            </section>
          ) : null}
          {!collectionsState.loading && !collectionsState.error && publicCollections.length === 0 ? <section className={cn(styles.empty)}><FolderHeart /><h3>还没有公开收藏夹</h3><p>前往“我的收藏夹”将一个收藏夹设为公开后，它会出现在这里。</p><Link href="/me/collections">管理收藏夹</Link></section> : null}

          <blockquote className={cn(styles.quote)}>“　收藏不是占有，是把值得回看的内容留在自己的星图里。　”</blockquote>
        </section>
      </main>
    </AppShell>
  );
