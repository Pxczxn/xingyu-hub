...
function Avatar({ creator }: { creator: CreatorCard }) { const name = creator.profile?.displayName || creator.displayName || creator.username; return <span className={cn(styles.realAvatar)}>{creator.profile?.avatar ? <img src={creator.profile.avatar} alt={`${name} 的头像`} /> : name.slice(0, 1).toUpperCase()}</span>; }
...
export default function CreatorsPage() {
  ...
  return <AppShell><main data-layout="creators" className={cn(styles.creatorsPage, styles.real)}>
    {error ? <Alert variant="destructive">{error}</Alert> : null}
    <section className={cn(styles.content)}><header><h1>推荐作者</h1><p>从真实社区专题中发现值得关注的创作者。</p></header><nav className={cn(styles.filters)} aria-label="按专题筛选作者"><button type="button" className={activeTopic === "ALL" ? styles.active : undefined} onClick={() => setActiveTopic("ALL")}>全部</button>{topics.map((topic) => <button type="button" className={activeTopic === topic.name ? styles.active : undefined} onClick={() => setActiveTopic(topic.name)} key={topic.id}>{topic.name}</button>)}<label><Search /><span className="sr-only">搜索作者</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索作者或专题" /></label></nav><div className={cn(styles.layout)}><section className={cn(styles.grid)}>{loading ? <p className={cn(styles.realStatus)}>正在加载作者…</p> : null}{visibleCreators.map((creator) => { const name = creator.profile?.displayName || creator.displayName || creator.username; const work = creator.works?.[0]; return <article key={creator.username}><header><Avatar creator={creator} /><div><h2>{name}<Star /></h2><p>{creator.topics.join(" · ") || "社区创作者"}</p></div></header><small>已发布内容</small><p className={cn(styles.tags)}><span>{creator.contentCount} 篇</span>{creator.topics.slice(0, 2).map((topic) => <span key={topic}>{topic}</span>)}</p>{work ? <Link href={`/articles/${encodeURIComponent(work.id)}`} className={cn(styles.work)}><span><small>最新公开作品</small><b>{work.title || "未命名作品"}</b><em>{work.categorySlug ? `分类：${work.categorySlug}` : "文章作品"}</em></span><i><FileText /></i></Link> : <div className={cn(styles.realWorkEmpty)}><FileText />暂未获得公开作品</div>}<footer><Link href={`/u/${encodeURIComponent(creator.username)}`}>查看主页</Link><button type="button" onClick={() => void toggleFollow(creator)}>{creator.profile?.following ? "已关注" : "关注"}</button></footer></article>; })}{!loading && !visibleCreators.length ? <div className={cn(styles.realStatus)}><UsersRound /><h2>暂无匹配作者</h2><p>调整专题或关键词后再试。</p></div> : null}</section><aside className={cn(styles.side)}><section><header><h2>按专题推荐</h2><Link href="/topics">更多 ›</Link></header>{topics.map((topic) => <Link href={`/topics/${encodeURIComponent(topic.slug)}`} key={topic.id}><i><Star /></i><span><b>{topic.name}</b><small>{topic.description || "社区专题"}</small></span><em>{typeof topic.contentCount === "number" ? `${topic.contentCount} 篇` : "查看 ›"}</em></Link>)}{!loading && !topics.length ? <p>暂无可展示专题。</p> : null}</section><section><h2>你可能感兴趣</h2>{creators.slice(0, 3).map((creator) => <Link href={`/u/${encodeURIComponent(creator.username)}`} key={creator.username}><Avatar creator={creator} /><span><b>{creator.profile?.displayName || creator.displayName || creator.username}</b><small>{creator.topics[0] || "社区创作者"}</small></span></Link>)}</section></aside></div></section>
  </main></AppShell>;
}
