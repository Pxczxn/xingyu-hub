...
export default function EventDetailPage() {
  ...
  if (loading) {
    return <AppShell><main className={cn(styles.detailReal, styles.detailRealLoading)} aria-busy="true">正在加载活动详情…</main></AppShell>;
  }
  if (!event) {
    return (
      <AppShell>
        <main className={cn(styles.detailReal, styles.detailRealMissing)} role="status">
  ...
  return (
    <AppShell>
      <main className={cn(styles.detailReal)}>
        <section className={cn(styles.detailRealHero)} aria-labelledby="event-title">
          ...
          <Button asChild className={cn(styles.detailRealHeroAction)}>
            <Link href={event.submissionOpen ? `/events/${encodeURIComponent(event.id)}/submit` : "/events"}>{event.submissionOpen ? "立即参与" : "返回活动广场"}</Link>
          </Button>
        </section>

        <div className={cn(styles.detailRealGrid)}>
          <section className={cn(styles.detailRealMain)}>
            <section className={cn(styles.detailRealPanel, styles.detailRealDescription)}>
            ...
            <section className={cn(styles.detailRealPanel, styles.detailRealFlow)}>
            ...
            <section className={cn(styles.detailRealPanel, styles.detailRealWorks)}>
              ...
              ))}</div> : <p className={cn(styles.detailRealPanelEmpty)}>首个投稿会在这里展示。</p>}
            </section>
          </section>

          <aside className={cn(styles.detailRealSide)}>
            <section className={cn(styles.detailRealPanel, styles.detailRealSubmissionList)}>
              <header><h2><UsersRound /> 最新投稿</h2><span>{submissions.length} 项</span></header>
              {submissions.length ? submissions.slice(0, 5).map((submission, index) => <p key={submission.id}><i>{index + 1}</i><span><b>{submission.objectTitle || "未命名作品"}</b><small>{statusLabel(submission.status)}</small></span><strong>{submission.objectType}</strong></p>) : <p className={cn(styles.detailRealSideEmpty)}>暂无投稿记录</p>}
              {event.submissionOpen ? <Button asChild><Link href={`/events/${encodeURIComponent(event.id)}/submit`}>提交我的作品</Link></Button> : null}
            </section>
            <section className={cn(styles.detailRealPanel, styles.detailRealMeta)}>
