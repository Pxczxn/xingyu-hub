...
export default function GalaxiesPage(){
  ...
  return <AppShell><main className="xy-galaxy-home">
    ...
    <div className="xy-galaxy-home-layout"><div>
      ...
      <div className="xy-galaxy-section-title"><h2><Sparkles/>共创成员</h2><small>成员按加入记录展示</small></div><div className="xy-galaxy-creators">{members.length?members.map(member=><Link href={`/u/${member.username}`} key={member.userId}><span>{(member.displayName||member.username).slice(0,1)}</span><b>{member.displayName||member.username}</b><small>{member.role||"成员"}</small></Link>):<p className="xy-galaxy-empty">暂无可展示的成员。</p>}</div>
