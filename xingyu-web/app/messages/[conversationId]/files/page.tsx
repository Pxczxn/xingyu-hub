...
export default function ConversationFilesPage() {
  ...
  const { data: messages, loading, error } = useAsyncData(
    () => communityApi.getConversationFiles(conversationId),
    [conversationId]
  );
    ...
    <AppShell>
      <main className="mx-auto max-w-lg px-4 py-8 sm:px-6">
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-xl font-semibold">会话文件</h1>
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground">暂无文件消息</p>
        ) : (
            ...
            {(messages ?? []).map((message) => (
              <li key={message.id}>
                <Card className="p-4">
                  ...
                  >
                    {message.attachmentName || message.body || "文件"}
                  </a>
                  {message.body && message.body !== message.attachmentUrl && (
