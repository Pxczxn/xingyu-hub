...
export default function MessagesEmptyPage() {
  return (
    <AppShell>
      <main className={cn(shellStyles.page, "max-w-4xl")}>
        <section className={cn(pageStyles.orbitBg, "rounded-[26px] border border-white/80 bg-white/72 p-6 shadow-[0_16px_42px_rgba(30,45,82,.08)] backdrop-blur-xl sm:p-8")}>
          <div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#eef1ff] text-[#5266b0]"><MessageSquare className="h-6 w-6" /></span><div><p className={pageStyles.kicker}>社区沟通</p><h1 className="mt-1 text-[28px] font-bold tracking-[-.04em] text-[#172b58]">从一次对话开始</h1><p className="mt-2 text-sm leading-6 text-[#69748a]">和同频的人分享想法，也可以创建一个小组一起交流。</p></div></div>
