...
export default function StudioPage() {
  ...
  async function createArticle() {
    setCreateError(null);
    try { const created = await communityApi.createArticle(); router.push(`/studio/content/${created.articleId}/edit`); }
  ...
  return <AppShell><main className="xy-studio-real">
    ...
    <section className="xy-studio-real-hero">
      <div className="xy-studio-real-user"><Avatar name={displayName} avatar={profile?.avatar} /><span><h1>欢迎回来，{displayName}</h1><p>继续把正在思考的内容写下来。</p><small>创作空间会自动汇总你的文章状态与社区互动。</small></span><Button variant="outline" asChild><Link href={profile?.username ? `/u/${profile.username}` : "/me"}>个人主页</Link></Button></div>
    ...
    <div className="xy-studio-real-layout"><section className="xy-studio-real-content">
      ...
      <div className="xy-studio-real-columns"><div>
        ...
        <section className="xy-studio-real-section xy-studio-real-performance"><header><h2>创作数据</h2><Link href="/studio/analytics">查看详情 <ArrowRight /></Link></header><div>{insights ? <><DataStat label="已发布文章" value={insights.articleCount} icon={<BookOpen />} /><DataStat label="获得喜欢" value={insights.likeCount} icon={<CheckCircle2 />} /><DataStat label="收到评论" value={insights.commentCount} icon={<MessageCircle />} /><DataStat label="粉丝" value={insights.followerCount} icon={<Sparkles />} /></> : <p>正在加载创作数据…</p>}</div></section>
      </div>
      <section className="xy-studio-real-section xy-studio-real-recent"><header><h2>最近编辑</h2><Link href="/studio/content">全部作品 <ArrowRight /></Link></header>{articlesState.loading ? <p className="xy-studio-real-status">正在加载作品…</p> : null}{!articlesState.loading && recent.length ? <div>{recent.map((article, index) => <Link href={`/studio/content/${encodeURIComponent(article.id)}`} key={article.id}><WorkMark index={index} /><span><b>{article.title || "未命名文章"}</b><small>文章 · {statusLabel[article.status] || article.status}</small></span><time>{formatDateTime(article.updatedAt)}</time></Link>)}</div> : null}{!articlesState.loading && !recent.length ? <p className="xy-studio-real-status">还没有创作内容，从第一篇文章开始吧。</p> : null}</section>
      </div>
    </section>
    <aside className="xy-studio-real-side"><section><h2>待处理事项</h2><div className="xy-studio-real-pending"><TriangleAlert /><p>{returned ? `有 ${returned} 篇内容需要处理审核反馈` : "暂无需要处理的审核反馈"}</p><Link href="/studio/content?tab=returned">查看</Link></div><div className="xy-studio-real-pending"><Clock3 /><p>{reviewing ? `${reviewing} 篇内容正在审核中` : "暂无审核中的内容"}</p><Link href="/studio/content?tab=reviewing">查看</Link></div></section><section><h2>草稿箱快捷访问</h2><div className="xy-studio-real-drafts"><Link href="/studio/content?tab=drafts"><FileText />文章草稿<b>{draft} 篇</b></Link><Link href="/studio/moments/new"><MessageCircle />发布动态<b>开始创作</b></Link><Link href="/studio/series"><Layers3 />系列管理<b>查看系列</b></Link></div></section></aside></div>
  </main></AppShell>;
}
