...
export function MissingScreen({ pathname }: { pathname: string }) {
  return <div className={shellStyles.page}><div className={cn(shellStyles.panel, "grid min-h-[55vh] place-items-center p-8 text-center")}><div><FileQuestion className="mx-auto h-10 w-10 text-[rgb(var(--violet))]" /><h1 className="mt-5 text-2xl font-semibold">这个坐标暂未开放</h1><p className="mt-3 text-sm text-muted-foreground">{pathname}</p><Button asChild className="mt-6"><Link href="/">返回首页</Link></Button></div></div></div>;
}

export function PendingState({ title }: { title: string }) {
  return <div className={cn(shellStyles.panel, "flex items-center gap-3 p-5 text-sm text-muted-foreground")}><LoaderCircle className="h-4 w-4 animate-spin" />正在准备{title}…</div>;
}
