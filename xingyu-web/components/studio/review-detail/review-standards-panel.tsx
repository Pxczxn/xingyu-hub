...
export function ReviewStandardsPanel() {
  return (
    <section className={cn(styles.standards)}>
      <details className={cn(styles.standardsDetails)}>
        <summary>
          <span>审核标准参考</span>
          <ChevronRight aria-hidden="true" />
        </summary>
        <div className={cn(styles.standardsBody)}>
          <p className={cn(styles.standardsTags)}>
            {REVIEW_KEYWORDS.map((keyword) => (
              <span key={keyword}>{keyword}</span>
            ))}
          </p>
          <p className={cn(styles.standardsCopy)}>
